...
`<link rel="canonical">` points to `kooraassess.houseofmastery.co` and `firsthour.houseofmastery.co/first-hour`. `<meta name="robots" content="index, follow, max-snippet:-1, max-image-preview:large">`. PWA manifest references the correct icon paths (`House-of-Mastery-with-Dr-Job-Mogire-favicon.png` at 192×192 and 512×512), correct theme_color, correct name/short_name. Five `<link rel="icon">` declarations span 32, 192, 180-touch, mask, shortcut.
